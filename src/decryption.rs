...
use num_traits::{ToPrimitive, Zero, Signed, One};
use std::sync::Arc;
...
fn extract_coefficient_as_poly(poly: &Poly, coeff_index: usize, params: &PvwParameters) -> Result<Poly> {
...
pub fn decrypt_party_shares(
    ...
    if all_ciphertexts.is_empty() {
        return Err(PvwError::InvalidParameters("No ciphertexts provided".to_string()));
    }
...
mod tests {
    ...
    fn create_larger_test_params() -> Arc<PvwParameters> {
        ...
        let (variance, bound1, bound2) = PvwParameters::suggest_correct_parameters(4, 4, 8, &moduli)
            .unwrap_or((1, 30, 60));
    ...
    fn test_basic_encrypt_decrypt_round_trip() {
        ...
        for (party_idx, &expected_scalar) in test_scalars.iter().enumerate() {
            let decrypted = decrypt_party_value(&ciphertext, &parties[party_idx].secret_key, party_idx).unwrap();
            assert_eq!(decrypted, expected_scalar, "Decryption failed for party {}", party_idx);
        }
    ...
    fn test_decrypt_all_values() {
        ...
        println!("✓ Decrypt all values test passed (party 0 correctly decrypted their value: {})", decrypted_all[0]);
    }
    ...
    fn test_single_scalar_values() {
        ...
        for test_value in [1u64, 2u64, 7u64, 13u64] {
            ...
            assert_eq!(decrypted, test_value, "Single scalar {} decryption failed", test_value);
        }
    ...
    fn test_larger_parameter_set() {
        ...
        for (party_idx, &expected) in test_scalars.iter().enumerate() {
            ...
            if decrypted == expected {
                ...
                println!("  Party {} decryption: expected {}, got {}", party_idx, expected, decrypted);
            }
        }
        ...
        assert!(success_rate >= 0.75, 
                "Success rate {:.1}% too low for larger parameters", success_rate * 100.0);

        println!("✓ Larger parameter set test passed (n={}, k={}, l={}) - Success rate: {:.1}%", 
                params.n, params.k, params.l, success_rate * 100.0);
    }
    ...
    fn test_multiple_encryptions_same_key() {
        ...
        for trial in 0..3 {
            ...
            for (party_idx, &expected) in test_scalars.iter().enumerate() {
                let decrypted = decrypt_party_value(&ciphertext, &parties[party_idx].secret_key, party_idx).unwrap();
                assert_eq!(decrypted, expected, "Trial {} party {} decryption failed", trial, party_idx);
            }
        }
    ...
    fn test_threshold_decrypt_basic() {
        ...
        println!("✓ Basic threshold decrypt test passed (correctly decrypted party 0's value: {})", decrypted_all[0]);
    }
    ...
    fn test_decrypt_party_shares() {
        ...
        let all_ciphertexts = crate::encryption::encrypt_all_party_shares(&all_shares, &global_pk, &mut rng).unwrap();
        assert_eq!(all_ciphertexts.len(), params.n);

        // Each party decrypts their designated shares from all dealers
        for party_idx in 0..params.n {
            let party_shares = decrypt_party_shares(&all_ciphertexts, &parties[party_idx].secret_key, party_idx).unwrap();
            ...
            for (dealer_idx, &decrypted_value) in party_shares.iter().enumerate() {
                ...
                assert_eq!(decrypted_value, expected_value, 
                          "Party {} failed to decrypt value from dealer {}", party_idx, dealer_idx);
            }
        }
    ...
    fn test_cross_party_decryption_fails() {
        ...
        assert_ne!(party_0_trying_to_decrypt_party_1, test_scalars[1], 
                  "Cross-party decryption should fail");

        // But party 1 should correctly decrypt their own value
        let party_1_correct_decryption = decrypt_party_value(
            &ciphertext,
            &parties[1].secret_key,
            1
        ).unwrap();
    ...
    fn test_decode_scalar_robust_public() {
        ...
        let original_u64 = if test_scalar >= 0 { test_scalar as u64 } else { 0 };
        assert!(decoded <= original_u64 + 10, "Decoded value {} too far from original {}", decoded, original_u64);
    ...
    fn test_optimized_decoding() {
        ...
        assert!(decoded <= original_u64 + 5, "Optimized decoding result {} too far from {}", decoded, original_u64);
    ...
    fn test_edge_case_values() {
        ...
        for (test_idx, test_scalars) in edge_cases.iter().enumerate() {
            ...
            for (party_idx, &expected) in test_scalars.iter().enumerate() {
                let decrypted = decrypt_party_value(&ciphertext, &parties[party_idx].secret_key, party_idx).unwrap();
                assert_eq!(decrypted, expected, "Edge case {} party {} failed", test_idx, party_idx);
            }
        }
    ...
    fn test_helper_functions() {
        ...
        let delta_squared_const = extract_constant_term_bigint(&delta_squared_poly, &params).unwrap();
    ...
    fn test_coefficient_extraction() {
        ...
        for (i, expected_coeff) in test_coeffs.iter().enumerate() {
            ...
            assert_eq!(&extracted_const, expected_coeff, "Coefficient {} extraction failed", i);
        }
        ...
        for (i, expected_coeff) in test_coeffs.iter().enumerate() {
            let extracted_const = extract_constant_term_bigint(&all_coeff_polys[i], &params).unwrap();
            assert_eq!(&extracted_const, expected_coeff, "All coefficients extraction failed at {}", i);
        }
    ...
    fn test_modular_operations() {
        ...
        assert!(reduced_const.abs() <= BigInt::from(10), "Modular reduction produced unexpected result: {}", reduced_const);
    ...
    fn test_invalid_party_index() {
        ...
        assert!(invalid_party_ct.is_none(), "Should return None for invalid party index");
        
        // Test accessing with valid index
        let valid_party_ct = ciphertext.get_party_ciphertext(0);
        assert!(valid_party_ct.is_some(), "Should return Some for valid party index");
    ...
    fn test_empty_secret_keys_threshold() {
        ...
        assert!(result.is_err(), "Threshold decrypt should fail with empty keys");
    ...
    fn test_decryption_consistency() {
        ...
        for (party_idx, &expected) in test_scalars.iter().enumerate() {
            let individual_result = decrypt_party_value(&ciphertext, &parties[party_idx].secret_key, party_idx).unwrap();
            assert_eq!(individual_result, expected, "Party {} couldn't decrypt their own value", party_idx);
        }
        ...
        assert_eq!(batch_results[0], test_scalars[0], "Batch decryption failed for party 0's designated value");
