...
impl PvwParametersBuilder {
    ...
    pub fn build(self) -> Result<PvwParameters> {
        ...
        println!(
            "[PVW_PARAMS] Using n={}, k={}, l={}, secret_variance={}",
            n, k, l, secret_variance
        );
        println!(
            "[PVW_PARAMS] Error bounds: bound_1={}, bound_2={}",
...
impl PvwParameters {
    ...
    pub fn gadget_polynomial(&self) -> Result<Poly> {
        println!(
            "[PVW_GADGET] Creating PVW gadget polynomial with Delta = {}",
            self.delta
        );
        ...
        for i in 0..self.l {
            ...
            if i < 4 {
                println!(
                    "[PVW_GADGET] g[X^{}] = Delta^{} = {}",
                    i, i, coefficients_bigint[i]
                );
            }
    ...
    pub fn gadget_vector(&self) -> Vec<BigUint> {
        ...
        for i in 0..self.l {
            ...
            if i < 4 {
                println!(
                    "[PVW_GADGET] gadget_vector[{}] = Delta^{} = {}",
                    i, i, delta_power
                );
            }
        ...
        println!(
            "[PVW_GADGET] Created gadget vector with {} elements",
            g.len()
        );
        g
    }
    ...
    pub fn gadget_element(&self) -> Vec<BigUint> {
        ...
        println!(
            "[PVW_GADGET] Created gadget element g = (Δ^{}, Δ^{}, ..., Δ, 1)",
            self.l - 1,
            self.l - 2
        );
        g
    ...
    pub fn bigints_to_poly(&self, bigints: &[BigInt]) -> Result<Poly> {
        ...
        println!(
            "[BIGINT_TO_POLY] Successfully converted {} BigInt coefficients using RNS reduction",
            bigints.len()
        );
    ...
    pub fn verify_correctness_condition(&self) -> bool {
        ...
        println!(
            "[VERIFY_CORRECTNESS] n={}, k={}, l={}",
            self.n, self.k, self.l
        );
        println!(
            "[VERIFY_CORRECTNESS] error_bound_1={}, error_bound_2={}",
            self.error_bound_1, self.error_bound_2
        );
        println!(
            "[VERIFY_CORRECTNESS] Delta^(l-1) = {}",
            self.delta_power_l_minus_1
        );
        println!(
            "[VERIFY_CORRECTNESS] First term (error_bound_1 * 24/sqrt(l²kn + 31.8kl)) = {:.6}",
            first_term
        );
        println!(
            "[VERIFY_CORRECTNESS] Second term (error_bound_2 * (2.37/sqrt(nl) + 1.7n)) = {:.6}",
            second_term
        );
        println!("[VERIFY_CORRECTNESS] Total bound = {:.6}", total_bound);
        println!(
            "[VERIFY_CORRECTNESS] Delta^(l-1) as f64 ≈ {:.6}",
            delta_power_f64
        );

        let condition_satisfied = delta_power_f64 > total_bound;
        println!(
            "[VERIFY_CORRECTNESS] Correctness condition satisfied: {}",
            condition_satisfied
        );
    ...
    pub fn suggest_correct_parameters(
        ...
        println!(
            "[SUGGEST_PARAMS] Coefficient for error_bound_1: {:.6}",
            coeff1
        );
        println!(
            "[SUGGEST_PARAMS] Coefficient for error_bound_2: {:.6}",
            coeff2
        );
