...
mod tests {
    ...
    fn test_pvw_parameters_serialization() {
        ...
        let reconstructed = deserialized.to_params().expect("Failed to reconstruct parameters");
    ...
    fn test_secret_key_serialization() {
        ...
        let reconstructed = deserialized.to_secret_key().expect("Failed to reconstruct secret key");
    ...
    fn test_public_key_serialization() {
        ...
        let reconstructed = deserialized.to_public_key().expect("Failed to reconstruct public key");
        
        // Verify properties
        assert_eq!(public_key.key_polynomials.len(), reconstructed.key_polynomials.len());
    ...
    fn test_global_public_key_serialization() {
        ...
        // Add a party key
        let secret_key = SecretKey::random(&params, &mut rng).expect("Failed to create secret key");
        global_pk.generate_and_add(0, &secret_key, &mut rng).expect("Failed to add party key");
        
        // Test serialization
        let serializable = SerializableGlobalPublicKey::from_global_public_key(&global_pk);
        let json = serde_json::to_string(&serializable).expect("Failed to serialize global public key");
        
        // Test deserialization
        let deserialized: SerializableGlobalPublicKey = 
            serde_json::from_str(&json).expect("Failed to deserialize global public key");
        let reconstructed = deserialized.to_global_public_key()
    ...
    fn test_ciphertext_serialization() {
        ...
        for i in 0..params.n {
            let secret_key = SecretKey::random(&params, &mut rng).expect("Failed to create secret key");
            global_pk.generate_and_add(i, &secret_key, &mut rng).expect("Failed to add party key");
        }
        ...
        let reconstructed = deserialized.to_ciphertext().expect("Failed to reconstruct ciphertext");
    ...
    fn test_round_trip_consistency() {
        ...
        let reconstructed1 = deserialized1.to_params().expect("Failed to reconstruct (1)");
        ...
        let reconstructed2 = deserialized2.to_params().expect("Failed to reconstruct (2)");
    ...
    fn test_json_serialization_format() {
        ...
        // Verify result
        let reconstructed_json = from_json.to_params().expect("Failed to reconstruct from JSON");
